...
# -----------------------------------EKS Configure IAM OIDC Provider Association --------------------------------------------------
...
# -----------------------------------EKS Configure IAM OIDC Provider Association For EFS --------------------------------------------------


resource "aws_iam_role" "eks_efs_role" {
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            "${aws_iam_openid_connect_provider.eks_oicd.url}:aud" = "sts.amazonaws.com",
            "${aws_iam_openid_connect_provider.eks_oicd.url}:sub" = "system:serviceaccount:kube-system:efs-csi-controller-sa"
          }
        }
      }
    ]
...
# Attach the managed policy for EFS CSI Driver
resource "aws_iam_role_policy_attachment" "efs_csi_driver_policy" {
  policy_arn = "arn:aws:iam::aws:policy/service-role/AmazonEFSCSIDriverPolicy"
  role       = aws_iam_role.eks_efs_role.name
  depends_on = [ aws_iam_role.eks_efs_role ]
}

# -----------------------------------Create EFS And Setup Security Group For EFS --------------------------------------------------
...
resource "aws_security_group" "efs_sg" {
  ...
  ingress {
    ...
    to_port     = 2049
    protocol    = "tcp"
    cidr_blocks = [aws_vpc.eks_vpc_deploy.cidr_block]
  }
